...
function SuggestBox() {
    ...
    // Твой API ключ от Кинопоиска (Вставь свой!)
    const KP_API_KEY = 'c2f87bc7-9924-41c2-aca6-bd9a6c014f9d' 

    // Логика клика снаружи (оставляем как было)
    useEffect(() => {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            // URL другой, и ключ передаем в заголовках (headers)
            ...
            // У Кинопоиска массив лежит в data.films
    ...
    // 2. ВЫБОР И ОТПРАВКА ПРЕДЛОЖЕНИЯ
    const handleSelectMovie = async (film) => {
        ...
        try {
            ...
            // --- ПРОВЕРКА 1: Основная таблица (по Году и Постеру) ---
            const checkMain = await supabase
                .from('movies')
                .select('posterUrl')
                .eq('year', searchYear); // Сначала фильтруем по году (это быстро)

            // Потом проверяем постеры в JavaScript (это надежно)
            if (checkMain.data) {
            ...
            // --- ПРОВЕРКА 2: Таблица предложений ---
            ...
            // Фикс для времени: API дает "02:09", база хочет "02:09:00"
            let formattedTime = null;
            if (film.filmLength && film.filmLength.includes(':')) {
                 formattedTime = film.filmLength.length === 5 ? film.filmLength + ":00" : film.filmLength;
            }
            ...
            const newOffer = {
                ...
                rating: film.rating && film.rating !== 'null' ? film.rating.toString() : "0", // Превращаем в строку, если в базе text
                        ...
                        {results.map((film) => (
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <small className="text-muted">{film.year}, {film.genres?.[0]?.genre}</small>
                                </div>
                            </li>
                        ))}
                    </ul>
